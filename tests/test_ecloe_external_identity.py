...
def test_external_login_provisions_only_synthetic_data_and_opaque_session() -> None:
    ...
    assert len(session["account"]["transactions"]) >= 2
...
def test_external_provisioning_is_deterministic_and_idempotent() -> None:
    ...
    first = repository.provision_external_user("https://tenant.ciamlogin.com/v2.0", key)
    second = repository.provision_external_user("https://tenant.ciamlogin.com/v2.0", key)

# 🏗️ Architecture — ECloe

ECloe is a low-cost adaptive experimentation MVP for financial next-best-action recommendations. It currently contains data ingestion/processing code, storage configuration shapes, tests, and documentation for the planned offline policy evaluation and demo interface.

## Overview

![Decision flow](docs/decision-flow.svg)

The system starts with a public Kaggle dataset, processes it into a leakage-safe local dataset, evaluates multiple recommendation policies offline, and prepares the selected policy for a small demonstrable interface. The decision flow above shows the intended runtime loop: a channel sends minimized context and eligible offers, the policy selects one offer, the decision is logged, and later rewards are linked back to the original `decision_id`.

## Components

| Component | Responsibility | Key files/paths |
|:---|:---|:---|
| Configuration | Loads local `.env` settings, data paths, Kaggle dataset slug, and Azure placeholders. | `src/core/config.py`, `.env.example` |
| Data ingestion | Downloads the configured Kaggle dataset into `data/raw/`. | `src/data/download.py` |
| Data processing | Normalizes columns, removes `duration`, maps `y` to binary rewards, and writes processed CSV output. | `src/data/process.py`, `tests/test_data_process.py` |
...
| Offline policy layer | Planned implementation for Baseline, Epsilon-Greedy, UCB, and Thompson Sampling evaluation. | Planned `src/bandits/`, `src/evaluation/` |
...
1. Download the Kaggle Bank Marketing dataset.
2. Process the dataset and remove leakage fields.
3. Simulate offer choices and binary rewards for three MVP offers.
4. Run the deterministic baseline and adaptive policies on the same offline sequence.
5. Log parameters, metrics, and artifacts locally with MLflow.
6. Select the policy for the Golden Set and demo interface.
...
The planned Decision API and reward payloads are documented in [`docs/api-contract.md`](docs/api-contract.md). The MVP can start with a local script or notebook, then expose the same request/response shape through FastAPI if needed for Demo Day.
...
- **Remove `duration`** — prevents temporal leakage from a post-contact field.
...
- The repository does not yet include implemented bandit policies, MLflow runs, notebooks, or a demo interface.
